...
function ImageUpdate(){
  ...
  useEffect(()=>{
    axios.get(`http://localhost/image/view.do?no=${no}&inc=0`)
    .then((response) =>{
      // 가져온 데이터 확인
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response =  await axios.post("http://localhost/image/update.do",vo);
      alert(response.data); // 처리 결과를 경고 창으로 띄운다.
      navigate(`/image/view?no=${no}&inc=0`); // 수정이 끝나면 글보기로 돌아간다.
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="button" className="btn btn-warning"
          onClick={()=>navigate(`/image/view?no=${no}&inc=0`)}>취소</button>
